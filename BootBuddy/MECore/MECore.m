...
- (void)setupSidebar11 {
    ...
    NSUInteger yLoc = _mainView.window.frame.size.height - height * 2 - 10;
...
- (IBAction)selectPreference:(id)sender {
    ...
    NSView *border = [_prefWindow performSelector:@selector(_borderView)];
    for (NSView *v in [border subviews]) {
        if ([[v className] isEqualToString:@"NSTitlebarContainerView"]) {
            titleView = [[v subviews] firstObject];
            for (NSView *v2 in v.subviews.firstObject.subviews) {
                if ([v2.className isEqualToString:@"_NSThemeZoomWidget"])
                    [v2 setHidden:true];
                if ([v2.className isEqualToString:@"_NSThemeWidget"])
                    [v2 setHidden:true];
            }
        }
    }
        
    CGFloat titleHeight = titleView.frame.size.height - 18;
    if (titleHeight == 0)
        titleHeight = _prefWindow.frame.size.height - contentHeight;
